...
while [ $# -gt 0 ]; do
	...
	if [ "$1" = "--" ]; then
		...
		get_data olc.tsv | awk -v cmd=list "$(get_data olc.awk)"
		exit
	elif [ "$1" = "-W" ]; then
		export olc_NOLINEWRAP=1
		shift
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data olc.tsv | awk -v cmd=ref -v ref="$ref" "$(get_data olc.awk)" | ${PAGER}
	done
	exit 0
fi

get_data olc.tsv | awk -v cmd=ref -v ref="$*" "$(get_data olc.awk)" | ${PAGER}
