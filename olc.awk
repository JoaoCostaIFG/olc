BEGIN {
	...
	MAX_WIDTH = 80
	if (ENVIRON["OLC_MAX_WIDTH"] ~ /^[0-9]+$/) {
		if (int(ENVIRON["OLC_MAX_WIDTH"]) < MAX_WIDTH) {
			MAX_WIDTH = int(ENVIRON["OLC_MAX_WIDTH"])
		}
	}
...
function printverse(verse,    word_count, characters_printed) {
	if (ENVIRON["OLC_NOLINEWRAP"] != "" && ENVIRON["OLC_NOLINEWRAP"] != "0") {
		printf("%s\n", verse)
		return
	}
